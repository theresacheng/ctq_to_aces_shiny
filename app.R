...
setwd("/Users/theresacheng/Box\ Sync/ctq-to-aces/ctq_to_aces_shiny")

# source helper functions
source("helper.R") #/Users/theresacheng/Box\ Sync/ctq-to-aces/ctq_to_aces_shiny

# load data and information about each subscale
df_items <- readRDS("df_items.rds")
subscale_info <- readRDS("subscale_info.rds")
  ...
  fluidPage( # fluid page -- app adjusts to browser window size, this is a want!
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        tags$li("Logistic regression: Predict ACE endorsement when the predicted probability of ACE endorsement (estimated from a logistic regression model) is above threshold"),
        br(),
        h4("Tables & Figures"),
        splitLayout(
          plotOutput("SSPN"),
